...
const PostQuizFeedbackModal = ({ attemptId }: Props) => {
  ...
  return <AnimatePresence>{open && <motion.div className="fixed inset-0 z-[120] flex items-center justify-center p-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
    ...
    <motion.div role="dialog" aria-modal="true" aria-labelledby="feedback-title" initial={{ opacity: 0, y: 24, scale: .96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 16, scale: .97 }} className="relative max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-[2rem] border border-white/30 bg-white/95 p-6 shadow-[0_30px_100px_-20px_rgba(5,7,11,.65)] backdrop-blur-2xl sm:p-8 dark:border-white/10 dark:bg-ink-950/95">
      ...
      <h2 id="feedback-title" className="mt-5 text-2xl font-display text-ink-950 dark:text-white">Rate your EraEdu experience</h2>
      ...
      <form onSubmit={submit} className="mt-6 space-y-5">
        <fieldset><legend className="text-sm font-semibold text-ink-800 dark:text-white">How was your quiz experience?</legend><div className="mt-2 flex gap-2" aria-label="Rating out of five">{[1,2,3,4,5].map((value) => <button key={value} type="button" onClick={() => setRating(value)} aria-label={`${value} star${value > 1 ? 's' : ''}`} aria-pressed={rating === value} className={`flex h-11 w-11 items-center justify-center rounded-xl text-xl transition ${rating >= value ? 'bg-primary-600 text-white shadow-md' : 'bg-primary-50 text-primary-300 ring-1 ring-primary-900/10 dark:bg-white/[.06] dark:text-white/30'}`}>★</button>)}</div></fieldset>
        ...
        <div className="flex flex-col-reverse gap-3 pt-2 sm:flex-row sm:justify-end"><button type="button" onClick={() => setOpen(false)} className="rounded-xl px-5 py-2.5 text-sm font-semibold text-ink-500 transition hover:bg-primary-50 dark:hover:bg-white/10">Not now</button><button disabled={submitting} type="submit" className="inline-flex items-center justify-center gap-2 rounded-xl bg-primary-700 px-5 py-2.5 text-sm font-semibold text-white shadow-lg transition hover:-translate-y-0.5 hover:bg-primary-800 disabled:opacity-50"><CheckCircleIcon className="h-5 w-5" />{submitting ? 'Sending...' : 'Send feedback'}</button></div>
      </form>
    </motion.div>

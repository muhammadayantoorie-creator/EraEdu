      ...
      {isOpen && quiz && (
        ...
        >
          ...
          <motion.div
            ...
            className="relative flex max-h-[94vh] w-full max-w-5xl flex-col overflow-hidden rounded-3xl border border-white/30 bg-white/90 shadow-[0_32px_100px_-20px_rgba(2,6,23,0.65)] backdrop-blur-2xl dark:border-white/10 dark:bg-slate-950/90"
          >
            ...
            {!isPreparing && <footer className="flex flex-col-reverse gap-3 border-t border-slate-200/70 bg-white/60 px-5 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-7 dark:border-white/10 dark:bg-white/[0.03]">
              {showFinalConfirmation ? (
                <>
                  <button type="button" onClick={() => setShowFinalConfirmation(false)} className="rounded-xl px-5 py-2.5 text-sm font-semibold text-slate-600 transition hover:bg-slate-200/70 focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:text-slate-300 dark:hover:bg-white/10">Go Back</button>
                  <button ref={continueButtonRef} type="button" onClick={() => setIsPreparing(true)} className="inline-flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-indigo-600 to-violet-600 px-6 py-2.5 text-sm font-bold text-white shadow-lg shadow-indigo-500/20 transition hover:-translate-y-0.5 hover:shadow-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">Continue<ArrowRightIcon className="h-4 w-4" /></button>
                </>
              ) : (
              <>
                <button type="button" onClick={onClose} className="rounded-xl px-4 py-2.5 text-sm font-semibold text-slate-600 transition hover:bg-slate-200/70 focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:text-slate-300 dark:hover:bg-white/10">Cancel</button>
                <div className="grid grid-cols-1 gap-2 sm:flex">
                <button type="button" onClick={onEdit} className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-300 bg-white/70 px-4 py-2.5 text-sm font-semibold text-slate-800 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:border-white/15 dark:bg-white/[0.06] dark:text-white"><PencilSquareIcon className="h-4 w-4" />Edit Quiz</button>
                <button type="button" onClick={() => setShowQuestions((value) => !value)} aria-expanded={showQuestions} className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-300 bg-white/70 px-4 py-2.5 text-sm font-semibold text-slate-800 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:border-white/15 dark:bg-white/[0.06] dark:text-white"><EyeIcon className="h-4 w-4" />{showQuestions ? 'Hide Preview' : 'Preview Quiz'}</button>
                <button type="button" onClick={handleOpen} disabled={!confirmed} aria-disabled={!confirmed} className="inline-flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-indigo-600 to-violet-600 px-5 py-2.5 text-sm font-bold text-white shadow-lg shadow-indigo-500/20 transition hover:-translate-y-0.5 hover:shadow-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 disabled:cursor-not-allowed disabled:from-slate-300 disabled:to-slate-300 disabled:text-slate-500 disabled:shadow-none disabled:hover:translate-y-0 dark:disabled:from-slate-800 dark:disabled:to-slate-800 dark:disabled:text-slate-500"><SparklesIcon className="h-4 w-4" />Open Quiz<ArrowRightIcon className="h-4 w-4" /></button>
                </div>
              </>
              )}
            </footer>}

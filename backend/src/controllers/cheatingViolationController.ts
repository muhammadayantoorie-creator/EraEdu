...
export const getViolations = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    const result = await cheatingViolationService.getViolationsForAttempt(attemptId, userId, userRole);
    res.status(200).json({ success: true, data: result });
  } catch (error: any) {
    const status = error.statusCode || (error.message.includes('authorized') ? 403 : 500);
...
export const flagAttempt = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    const result = await cheatingViolationService.flagAttempt(attemptId, userId, userRole);
    res.status(200).json({ success: true, data: result });
  } catch (error: any) {
    res.status(error.statusCode || 500).json({ success: false, error: { message: error.message } });
  }
...
export const invalidateAttempt = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    const result = await cheatingViolationService.invalidateAttempt(attemptId, userId, userRole);
